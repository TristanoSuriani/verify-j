...
public class DefaultRuntime<C> implements SimulationRuntime<C> {
    private final Specification<C> specification;
    private final SimulationOptions options;

    public DefaultRuntime(Specification<C> specification, SimulationOptions options) {
    ...
    @Override
    public Specification<C> specification() {
        return specification;
    }

    public Outcome run() {
        C context = tryInit(specification.init(), 100);
    ...
    private boolean[] initialiseInvariantSatisfactionVector() {
        boolean[] invariantSatisfied = new boolean[specification.invariants().length];
        for (int i = 0; i < invariantSatisfied.length; i++) {
            var invariant = specification.invariants()[i];
